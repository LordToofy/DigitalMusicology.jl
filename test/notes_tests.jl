...
@testset "quantize" begin
    ...
    @test onset(quantize(znote,1//2)) ==  0 && offset(quantize(znote,0.5)) ==  0
    @test onset(quantize(note1,1)) ==  0 && offset(quantize(note1,1)) ==  1
    @test onset(quantize(note2,1//2)) ==  0 && offset(quantize(note2,0.5)) ==  1
    @test onset(quantize(note3,1//2)) ==  0 && offset(quantize(note3,0.5)) ==  0.5
    @test onset(quantize(note4,1//10)) == 28.3
    @test offset(quantize(note4,1//10)) ==  41.4   #WANRING with thresholds like 0.1, return non-exactly rounded values like 1.000005
end
